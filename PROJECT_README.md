...
**功能说明：**
- 同步节假日数据（自动检查并获取缺失的节假日数据）
- 删除旧数据（3个月前的历史数据 + 非自选基金的数据）
- 初始化基金数据（实时数据 + 历史数据）
  - 从实时接口获取基金最新价格和名称
  - 从历史接口获取基金历史净值数据（增量更新）

**建议调用频率：** 每10分钟调用一次，保持数据最新

**响应示例：**
```json
{
  "code": 200,
  "message": "初始化任务已启动，正在后台执行，请查看日志了解进度",
  "data": null
}
```
...
- 实际的数据获取和更新在后台线程中执行
- 可以通过查看日志了解执行进度和结果
- 所有数据更新都基于自选基金表（t_fund_self）中的基金代码
